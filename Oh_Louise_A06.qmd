...
<https://github.com/louiseoh2025/Oh_Louise_201_A06.git>
:::
...
```{r}
# initialize an empty list for points per game
...
```{r}
# define a customized rounding function
custom_round <- function(x) {
  if (x < 100) {
    return(round(x, -1)) # round to nearest tens
  } else if (x < 1000) {
    return(round(x, -2)) # round to nearest hundreds
  } else {
    return(round(x, -3)) # round to nearest thousands
  }
}

# custom rounding function to each element of the matrix
mat_rounded <- apply(mat_rand, MARGIN = 1:2, custom_round)

# print matrices
print(mat_rand) # original
print(mat_rounded) # rounded
```
...
```{r}
# slice to only include Colorado (CO)
colorado_temps <- max_temps |> 
  filter(state == "CO")

# slice to only include Ohio (OH)
ohio_temps <- max_temps |> 
  filter(state == "OH")

# slice to only include Virginia (VA)
virginia_temps <- max_temps |> 
  filter(state == "VA")
```
...
colorado_median_annual <- colorado_temps |>
  group_by(year) |>
  summarise(median_annual_temp = median(c_across(jan:dec), na.rm = TRUE))

# max median temperature
max_median_temp <- max(colorado_median_annual$median_annual_temp)
max_median_year <- colorado_median_annual$year[
  which.max(colorado_median_annual$median_annual_temp)
  ]

# min median temperature
min_median_temp <- min(colorado_median_annual$median_annual_temp)
min_median_year <- colorado_median_annual$year[
  which.min(colorado_median_annual$median_annual_temp)
  ]

# print result
print(paste0("The maximum median annual temperature of ", 
             max_median_temp, " occurred in ", max_median_year, 
             " and the minimum median annual temperature of ", 
             min_median_temp, " occurred in ", min_median_year, "."))
```
...
```{r}
# recursive function to calculate the sum of the digits in a number
...
# run the function with 935068
sum_of_digits(935068) # output should be 31
```
